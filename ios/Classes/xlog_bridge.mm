...
#ifndef FLUTTER_XLOG_HAS_MARS
#define FLUTTER_XLOG_HAS_MARS 0
#endif
...
// Mars 的 iOS 静态库仍引用 OpenSSL 风格的 MD5 符号。
// 这里用 CommonCrypto 提供同 ABI shim，避免动态 framework 再去链接
// 平台不匹配的 openssl_lib_iOS/libcrypto.a。
thread_local unsigned char g_md5_buffer[CC_MD5_DIGEST_LENGTH];
}

extern "C" unsigned char* MD5(const unsigned char* data, size_t len, unsigned char* md) {
    unsigned char* output = md == nullptr ? g_md5_buffer : md;
    return CC_MD5(data, CC_LONG(len), output);
}
#endif
